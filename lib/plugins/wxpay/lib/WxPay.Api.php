...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsOut_trade_noSet()) {
			Log::DEBUG("ddd:缺少统一支付接口必填参数out_trade_no");
			exit("缺少统一支付接口必填参数out_trade_no！");
		}else if(!$inputObj->IsBodySet()){
			Log::DEBUG("ddd:缺少统一支付接口必填参数body");
			exit("缺少统一支付接口必填参数body！");
		}else if(!$inputObj->IsTotal_feeSet()) {
			Log::DEBUG("ddd:缺少统一支付接口必填参数total_fee");
			exit("缺少统一支付接口必填参数total_fee！");
		}else if(!$inputObj->IsTrade_typeSet()) {
			Log::DEBUG("ddd:缺少统一支付接口必填参数trade_type");
			exit("缺少统一支付接口必填参数trade_type！");
		}
		
		//关联参数
		if($inputObj->GetTrade_type() == "JSAPI" && !$inputObj->IsOpenidSet()){
			Log::DEBUG("ddd:统一支付接口中，缺少必填参数openid！trade_type为JSAPI时，openid为必填参数！");
			exit("统一支付接口中，缺少必填参数openid！trade_type为JSAPI时，openid为必填参数！");
		}
		if($inputObj->GetTrade_type() == "NATIVE" && !$inputObj->IsProduct_idSet()){
			Log::DEBUG("ddd:统一支付接口中，缺少必填参数product_id！trade_type为JSAPI时，product_id为必填参数！");
	...
	public static function notify($callback, &$msg)
	{
        ...
        //file_put_contents('xxx2.txt',var_export($xml,true));
		//var_dump($xml);
		//如果返回成功则验证签名
		try {
