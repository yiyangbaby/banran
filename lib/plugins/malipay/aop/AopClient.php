...
class AopClient {
	...
	protected function getSignContent($params) {
		...
		foreach ($params as $k => $v) {
			if ($k != 'site' && $k != 'case' && $k != 'act' && $k != 'code' && false === $this->checkEmpty($v) && "@" != substr($v, 0, 1)) {

				// 转换成目标字符集
				$v = $this->characet($v, $this->postCharset);
                if($k != 'sign'){
                    $v = stripslashes(html_entity_decode(urldecode($v)));
                }
				if ($i == 0) {
	...
	*/
	...
	protected function buildRequestForm($para_temp) {
        ...
        $sHtml = $sHtml."<input type='submit' value='手机支付宝支付' /></form>";
	...
	function verify($data, $sign, $rsaPublicKeyFilePath, $signType = 'RSA') {
        ...
        //@file_put_contents('logs_data.txt',var_export($data,true));
